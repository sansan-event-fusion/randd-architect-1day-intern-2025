...
# ボタンを押したらAPIを呼び出す
if st.button("検索"):
    # 入力されたuser_idを使用してAPIを呼び出すlimitを10に設定
    owner_url=f"https://circuit-trial.stg.rd.ds.sansan.com/api/contacts/owner_users/{input_user_id}?limit=100&start_date={input_date}"
    owner_all_url = f"https://circuit-trial.stg.rd.ds.sansan.com/api/contacts/owner_users/{input_user_id}?limit=100"
    try:
        response = requests.get(owner_url, timeout=30)
        ...
        #やりとりをしたユーザーの情報を取得
        all_response = requests.get(owner_all_url, timeout=30)
        all_response.raise_for_status()  # エラーハンドリング
        all_data = all_response.json()
        all_df = pd.DataFrame(all_data)
        all_user_ids = all_df["user_id"].unique()  # 全てのユニークな user_id を取得
        user_info_list = []
        for user_id in user_ids:
            user_info_url = f"https://circuit-trial.stg.rd.ds.sansan.com/api/cards/{user_id}"
            try:
                user_info_response = requests.get(user_info_url, timeout=30)
        ...
        for user_id in user_ids:
            ...
            try:
                similer_response = requests.get(similer_user_url, timeout=30)
                ...
                # all_user_idにない類似ユーザーの情報を表示
                similer_df = similer_df[~similer_df["user_id"].isin(all_user_ids)]
